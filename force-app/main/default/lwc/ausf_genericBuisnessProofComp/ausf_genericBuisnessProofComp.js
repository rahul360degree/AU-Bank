...
export default class Ausf_genericBuisnessProofComp extends LightningElement {
    ...
    @api componentLabelName = 'GST Registration Certificate';
    //address modal
    ...
    handleAllInputValues() {
       ...
       let allFilled =true;
        ...
        allFilled =  (allFilled && this.activity && this.subIndustry && this.industry && this.sector && (this.fileDataList.length > 0)) ? true : false ;
    ...
    createEmploymentDetails() {
        ...
        let employmentObj = {
            ...
            'Industry_Type__c': this.industryType,
            'Type_Verification__c':'Manual Upload'
        }
    ...
    handleAddressSubmit(event) {
        ...
        createEmploymentDetails({ employmentDetObj: JSON.stringify(employmentObj), screenName: this.screenName, recordTypeName: this.employeeRecordType })
            .then(result => {
                if (result) {
                    this.employmentDetId = result;
                    console.log('employmentd details update successfully ', this.employmentDetId);
                }
                this.showLoader = false;
            })
            .catch(error => {
